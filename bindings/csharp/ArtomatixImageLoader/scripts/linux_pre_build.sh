...
if [ -z "$PRE_BUILD_NO_TERM" ]; then
    STATUSFILE=$(mktemp)
    $termCommand "PRE_BUILD_NO_TERM=true \"$0\" \"$1\" \"$2\"; EXIT_STATUS=\$?; echo; echo EXIT STATUS: \$EXIT_STATUS; echo \$EXIT_STATUS > $STATUSFILE; read -n1 -r -p 'Press any key to continue...'"
